...
{
    /// <summary>
    /// Class to support Blowfish encryption/decryption algorithm.
    /// </summary>
    public sealed class CryptBlowfish : IDisposable
    {
        #region Properties
        /// <summary>
        ...
        // Rounds used in algorithm (btw, KeyBytes is 8)
        ...
        // Dynamic tables
        private uint[] P { get; set; } = null;
        private uint[,] S { get; set; } = null;
        #endregion
        ...
        /// </summary>
        /// <param name="key">The key to cipher/decipher with, as byte array.</param>
        public CryptBlowfish(byte[] key)
        {
            InitCryptBlowfish(key);
        }
        /// <summary>
        /// Constructs and initializes a blowfish instance with the supplied key.
        /// </summary>
        /// <param name="key">The key to cipher/decipher with, as string</param>
        public CryptBlowfish(string key)
        {
            InitCryptBlowfish(Encoding.ASCII.GetBytes(key));
        }
        /// <summary>
        /// Constructs and initializes a blowfish instance with the supplied key.
        /// </summary>
        ...
        /// <summary>
        /// Encrypt a plain text using Blowfish.
        /// </summary>
        /// <param name="plaintext">Input string to be encrypted</param>
        /// <returns>Output string with encrypted text</returns>
        public string Encrypt(string plaintext)
        {
            // Data must be a multiplier of 8
            var inputtext = plaintext;
            var padding   = inputtext.Length % 8;
            // if not, pad it with spaces
            if (padding != 0)
            {
                padding = 8 - padding;
            ...
            return Convert.ToBase64String(Encipher(Encoding.ASCII.GetBytes(inputtext)));
        }
        /// <summary>
        /// Decrypt a text using Blowfish
        /// </summary>
        /// <param name="plaintext">Input string with encrypted text</param>
        /// <returns>Output string with decrypted text</returns>
        public string Decrypt(string plaintext)
        {
            var outputtext = plaintext;
            var padding    = outputtext.Length % 8;
            // I think this won't be necessary, but just in case
            if (padding != 0)
            {
                padding = 8 - padding;
                for (var i = 0; i < padding; i++)
            ...
            var result = Encoding.ASCII.GetString(Decipher(Convert.FromBase64String(outputtext)));
        ...
        /// <summary>
        /// Cypher block for an input unint
        /// </summary>
        /// <param name="x">Input data to cypher (uint)</param>
        /// <returns>Cypher output data (uint)</returns>
        private uint F(uint x)
        {
            ushort a = (ushort)((x & 0xFF000000) >> 24);
            ushort b = (ushort)((x & 0x00FF0000) >> 16);
            ushort c = (ushort)((x & 0x0000FF00) >> 8);
            ushort d = (ushort)(x & 0x000000FF);
        ...
        {
            if ((data.Length % 8) != 0)
                throw new Exception("Invalid Encipher Length");
            ...
            for (int i = 0; i < length; i += 8)
            {
                ...
                output[i + 0] = (byte)(xl >> 24);
                output[i + 1] = (byte)(xl >> 16);
                output[i + 2] = (byte)(xl >> 8);
                output[i + 3] = (byte)(xl);

                output[i + 4] = (byte)(xr >> 24);
                output[i + 5] = (byte)(xr >> 16);
                output[i + 6] = (byte)(xr >> 8);
                output[i + 7] = (byte)(xr);
            }
            return output;
        }
        ...
        {
            if ((data.Length % 8) != 0)
                throw new Exception("Invalid Decipher Length");
